...
**VPS (2026-05-15, NL turn isolation rollout):** **148.253.209.54**, `/opt/pb-studio/pb-memoh-studio` — **HEAD `e0e75fd`** (≥ `cf636d96`); без сноса Postgres/Redis volumes; **studio-api/worker/beat** и **Memoh server** пересобраны и перезапущены; health **jar** / **memo** — **200**; **getMe** — **jarvispbweb_bot**; webhook **url пустой**; **pending_update_count=0**; **`vps-e2e-smoke.sh`** — **PASS**, ожидаемые **SKIP** (history, live telegram, prod pytest). Живой сценарий «чаты → отчёт → запомни → модель» — **после сообщений оператором**.
