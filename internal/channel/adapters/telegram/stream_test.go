...
func TestGroupFinalOnly_DeltasDoNotSendOrEdit(t *testing.T) {
	t.Parallel()

	adapter := NewTelegramAdapter(nil)
	s := &telegramOutboundStream{
		adapter:        adapter,
		cfg:            channel.ChannelConfig{ID: "test", Credentials: map[string]any{"botToken": "fake"}},
		target:         "-100",
		isPrivateChat:  false,
		groupFinalOnly: true,
	}
	...
	sendTextForTest = func(_ *tgbotapi.BotAPI, _ string, _ string, _ int, _ string) (int64, int, error) {
		t.Fatal("sendMessage must not run on group delta when groupFinalOnly")
		return 0, 0, nil
	}
	sendEditForTest = func(_ *tgbotapi.BotAPI, _ tgbotapi.EditMessageTextConfig) error {
		t.Fatal("editMessageText must not run on group delta when groupFinalOnly")
		return nil
	}
	defer func() {
		getOrCreateBotForTest = origGetBot
	...
	for _, d := range []string{"a", "b", "c"} {
		if err := s.Push(ctx, mustPreparedTelegramEvent(t, channel.StreamEvent{Type: channel.StreamEventDelta, Delta: d})); err != nil {
			t.Fatalf("delta: %v", err)
		}
	}
	s.mu.Lock()
	got := strings.TrimSpace(s.buf.String())
	s.mu.Unlock()
	if got != "abc" {
		t.Fatalf("buffer expected abc, got %q", got)
	}
}

func TestGroupFinalOnly_FinalSingleSendNoEllipsis(t *testing.T) {
	t.Parallel()
	...
	err := s.Push(ctx, mustPreparedTelegramEvent(t, channel.StreamEvent{
		Type: channel.StreamEventFinal,
		Final: &channel.StreamFinalizePayload{
			Message: channel.Message{Text: "Hello final"},
		},
	}))
	if err != nil {
	...
	if len(bodies) != 1 {
		t.Fatalf("expected 1 sendMessage, got %d bodies %#v", len(bodies), bodies)
	}
	if strings.Contains(bodies[0], "……") {
		t.Fatalf("final text must not contain pending ellipsis suffix: %q", bodies[0])
	}
}

func TestGroupFinalOnly_DuplicateFinalSendsOnce(t *testing.T) {
	t.Parallel()
	...
	finalEv := channel.StreamEvent{
		Type: channel.StreamEventFinal,
		Final: &channel.StreamFinalizePayload{
			Message: channel.Message{Text: "same"},
		},
	}
	if err := s.Push(ctx, mustPreparedTelegramEvent(t, finalEv)); err != nil {
	...
	if sendCount != 1 {
		t.Fatalf("expected 1 sendMessage for duplicate finals, got %d", sendCount)
	}
}
