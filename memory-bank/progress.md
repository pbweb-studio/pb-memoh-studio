...
| MVP v1 PR1 — role-aware: 9 MCP tools, skill v2, legacy NL responder удалён | Done (commit **`db9c1b6e`**, выкат на VPS 148.253.209.54: Studio 6 контейнеров healthy, MCP `tools/list` = **20 инструментов**, Memoh restart picks up skill, тесты 320 passed) |
| Orphan unanswered user-turn 16:54 (source_message_id=703) — лечение off-by-one | Done (data fix: `DELETE FROM bot_history_messages WHERE id LIKE 'cce790a0-%'`, история восстановлена) |
| DM hygiene 21:30 MSK: «Вижу: …» паразит + склейка ответов (sess `cf704360-…`) | Done (skill v2.1 правила анти-склейки + soft-delete сессии 114 строк + watchdog systemd timer; ADR в `docs/06_DECISIONS.md`) |
| Long-term root fix Memoh-core: failed generation должен удалять/помечать user-turn | Pending — PR2 + отдельный ADR (см. правило `040-no-core-damage.mdc`) |
| PR2 — UX-фичи U1–U8 (self-intro, auto-suggest, briefing, inline confirms, voice, mute, wizard, навигация) | Pending |
| 6+ Сводки (LLM / доставка в Telegram) и прочее | Pending |

**Последнее (2026-05-15 ~21:40 MSK):** второй инцидент в DM — пользователь видел «бот опять лагает / отвечает на N-1». Диагностика: orphan-ов в БД нет, все user/assistant пары парны. Корень — **ни разу не сбрасывалась DM-сессия `cf704360-…`** (114 строк за 2 дня): модель один раз сгаллюцинировала названия групп и зафиксировала шаблон «Вижу: личку, Управление Jarvis, PBVOICE» в каждом ответе, плюс склеивала прошлый отчёт в начало нового ответа. Лечение: (1) immediate — soft-delete сессии + DELETE 114 строк (бэкап в `/opt/pb-studio/backups/memoh-dm-reset/`); (2) `skills/pb-studio-manager/SKILL.md` дополнен разделом «Анти-галлюцинации и анти-склейка ответов»; (3) systemd-watchdog `memoh-orphan-cleanup.{service,timer}` против будущих orphan user-row. ADR в `docs/06_DECISIONS.md` секция «Memoh DM history hygiene (orphan watchdog + skill anti-coalescing)». Memoh-core не трогали. Деплой выполнен. Подтверждение от пользователя — после серии новых сообщений в DM (см. чеклист в `docs/AI_CONTEXT.md` §11).
